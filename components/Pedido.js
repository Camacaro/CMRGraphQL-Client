...
export const Pedido = ({id, total, cliente: { nombre, apellido, email, telefono, id: idCliente }, estado, pedido}) => {
  ...
  const [estatdoPedido, setEstatdoPedido] = useState(estado)

  const [clase, setClase] = useState('')

  useEffect(() => {
    if(estatdoPedido){
      setEstatdoPedido(estatdoPedido)
    }
    clasePedido()
  }, [estatdoPedido])

  // Funcion que modiufica el color del pedido de acuerdo a su estado
  const clasePedido = () => {
    if( estatdoPedido === 'PENDIENTE' ) {
      setClase('border-yellow-500')
    } else if ( estatdoPedido === 'COMPLETADO') {
  ...
  const cambiarEstadoPedido = async (event) => {
    ...
    try {
      ...
      setEstatdoPedido(data.actualizarPedido.estado)
